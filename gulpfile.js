    ...
    return gulp
      ...
      .src(mainBowerFiles({
        ...
        "overrides": {
          ...
          "lodash":              { "main": "dist/lodash.min.js" }
        }
      }))
      // .pipe(logger({ before: '[bower:js] ' }))
      .pipe(rename({ extname: '' }))
      .pipe(rename({ extname: '' }))
...
gulp.task('orig:min', ['orig:js:min', 'orig:css']);
  ...
  gulp.task('orig:js:babel', function(cb) {
    return gulp
      .src('sources/js/*')
      .pipe(babel({ presets: ['es2015'] }))
      // .pipe(uglify())
  ...
  gulp.task('orig:js:min', function(cb) {
    return gulp
      .src('sources/js/*')
      .pipe(babel({ presets: ['es2015'] }))
      .pipe(uglify())
